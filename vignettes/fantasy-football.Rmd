...
an "lid" global option. This option can be easily set using the `set_lid()`
function, which uses `options()` to save your league ID in your R profile which
...
ESPN analysts publish weekly and pre-season outlooks on most players.
...
zero power wins and the highest get's one less than the number of teams.
